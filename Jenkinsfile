    ...
    post {
        always {
            script {
                    ...
                    failedRows += """
                        <tr style="background:${bg}">
                          ...
                          <td style="padding:11px 16px;color:#888;font-size:12px;
                                     border-bottom:1px solid #2d0000;white-space:nowrap">[${fileLbl}]</td>
                ...
                // ── 3. Failed section (only shown when there are failures) ────
                def failedSection = ''
                if (failed > 0) {
                    failedSection = """
...
<table width="100%" cellpadding="0" cellspacing="0">
  <tr>
    <td style="background:linear-gradient(135deg,#1c0000 0%,#2d0000 40%,#0a0a0a 100%);
      ...
      <div style="margin-top:22px;font-size:20px;font-weight:700;color:#e0e0e0">API Test Report</div>
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#111">
  <tr>
    <td style="padding:30px 40px">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="text-align:center;padding:22px 10px;background:#0d0d0d;border:1px solid #1e1e1e;border-radius:4px">
            <div style="font-size:44px;font-weight:900;color:#e0e0e0;line-height:1">${total}</div>
            <div style="font-size:10px;color:#444;letter-spacing:3px;text-transform:uppercase;margin-top:8px">Total</div>
          </td>
          <td style="width:10px"></td>
          <td style="text-align:center;padding:22px 10px;background:#001500;border:1px solid #003300;border-radius:4px">
            <div style="font-size:44px;font-weight:900;color:#33cc33;line-height:1">${passed}</div>
            <div style="font-size:10px;color:#1a5c1a;letter-spacing:3px;text-transform:uppercase;margin-top:8px">Passed</div>
          </td>
          <td style="width:10px"></td>
          <td style="text-align:center;padding:22px 10px;background:#1a0000;border:1px solid #330000;border-radius:4px">
            <div style="font-size:44px;font-weight:900;color:#CC0000;line-height:1">${failed}</div>
            <div style="font-size:10px;color:#5c1a1a;letter-spacing:3px;text-transform:uppercase;margin-top:8px">Failed</div>
          </td>
          <td style="width:10px"></td>
          <td style="text-align:center;padding:22px 10px;background:#0f0f0f;border:1px solid #1e1e1e;border-radius:4px">
            <div style="font-size:44px;font-weight:900;color:#444;line-height:1">${skipped}</div>
            <div style="font-size:10px;color:#333;letter-spacing:3px;text-transform:uppercase;margin-top:8px">Skipped</div>
          </td>
          <td style="width:10px"></td>
          <td style="text-align:center;padding:22px 10px;background:#0d0d0d;border:1px solid #1e1e1e;border-radius:4px">
            <div style="font-size:44px;font-weight:900;line-height:1;
                        color:${passRate == 100 ? '#33cc33' : passRate >= 80 ? '#ccaa00' : '#CC0000'}">${passRate}%</div>
            <div style="font-size:10px;color:#444;letter-spacing:3px;text-transform:uppercase;margin-top:8px">Pass Rate</div>
          </td>
        </tr>
      </table>
    </td>
